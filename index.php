...
$audioFiles = [];
if (is_dir($musicaDir)) {
    $audioFiles = array_filter(scandir($musicaDir), function ($file) {
...
<div class="option-group">
    <h3>🎵 Musica di sottofondo (opzionale)</h3>

    <label for="audio">Seleziona un brano:</label>
    <select name="audio" id="audio">
        <option value="">-- Nessun brano selezionato --</option>
        <?php foreach ($audioFiles as $file): ?>
            <option value="<?= htmlspecialchars($file) ?>"><?= htmlspecialchars($file) ?></option>
        <?php endforeach; ?>
    </select>

    <label for="volume">Volume (da 0.0 a 2.0):</label>
    <input type="number" name="volume" id="volume" step="0.1" min="0" max="2" value="1.0" style="width: 80px;">

    <label for="start_time">Taglio inizio audio (sec):</label>
    <input type="number" name="start_time" id="start_time" min="0" step="1" value="0" style="width: 80px;">

    <label for="end_time">Taglio fine audio (sec):</label>
    <input type="number" name="end_time" id="end_time" min="0" step="1" value="0" style="width: 80px;">
</div>
